...
zz.storeEvents = zz.Class.extend({
    on: function(id, event, fn, self){
        if (!this.store[id]){
            this.store[id] = [];
        }
        
        this.store[id].push(
            this.target.on(event, fn, self)
        );
    },
    off: function(id){
        if (this.store[id]){
            this.store[id].forEach(function(e){
                this.target.off(e);
            }.bind(this));
...
zz.event = zz.Class.extend({
    initialize: function (){
        ...
        this.on = this.addEventListener = function(eventName, fn, self, silence){
            ...
            var ev = {fn:fn,self:self,ign:silence,n:eventName};

            events[eventName].push(ev);
